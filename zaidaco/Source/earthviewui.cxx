...
extern void load_data_sets_cb(Fl_Widget *o, void*v)
{
...
#ifdef __APPLE__
	static char *file = getcwd(dir, MAXPATHLEN - 1);
#else
	static char *file;
#endif
    ...
    Fl_File_Chooser chooser(file,                        // directory
                            "*.xyz",                        // filter
                            Fl_File_Chooser::MULTI,     // chooser type
                            "Choose Data Sets - hold shift key for more than one");        // title
    chooser.show();
	...
	Fl::check();                     
	Fl_Window *w = new Fl_Window(200, 120, 300, 80, FILE_LOADER_LOADING_FILES);
	w->begin();                    
    ...
    if ( chooser.count() > 0 ) {
        for ( int t=0; t<=chooser.count(); t++ ) {
			...
			char * file_name = strdup(chooser.value(t));
			
			
			if(ds->load_file(file_name))
			{
				map->initialize_arrays(ds->index());
				set_frame(ds->index());
				double 	mean = 	ds->get_current()->mean();
				...
				map->reset_vertices();
				//fname->value(message);
				progress->value((float) t/(float) chooser.count()); 
				Fl::check(); 
			...
			else
			{
				...
				//fl_alert(MAIN_UI_FILE_TYPE_ERROR, file_name);
			}
